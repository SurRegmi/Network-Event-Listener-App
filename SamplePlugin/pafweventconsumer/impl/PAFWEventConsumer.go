...
func LoadPAFWConfig() (config.PAFWConfig, error) {
  ...
  // Decode company Prism Base64 encoded password.
  // Note : Developers can exercise their own encryption mechanism for credentials.
  decoded, err = base64.StdEncoding.DecodeString(pafwConfig.companyClusterConfig.Password)
  if err != nil {
    glog.Error("Decode error:", err)
  } else {
    pafwConfig.companyClusterConfig.Password = string(decoded)
  }
  if err != nil {
    glog.Error("Decode error:", err)
  }
