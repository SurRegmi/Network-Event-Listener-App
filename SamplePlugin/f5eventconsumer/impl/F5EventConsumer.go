...
func LoadF5Config() (config.F5Config, error) {
  ...
  // Decode  Prism Base64 encoded password.
  // Note : Developers can exercise their own encryption mechanism for credentials.
  decoded, err = base64.StdEncoding.DecodeString(f5Config.companyClusterConfig.Password)
  if err != nil {
    glog.Error("Decode error:", err)
  } else {
    f5Config.companyClusterConfig.Password = string(decoded)
  }
  if err != nil {
    glog.Error("Decode error:", err)
  }
